...
use cfg::{build_cfgs_for_file, parse_project_with_options, ParseOptions};
...
use crate::instrumentation::{
    instrument_for_branch_coverage, modify_source_files, InstrumentationData,
};
...
pub fn analyze_branch_coverage_with_options(
    ...
    // Parse project to get CFGs with filtering options
    ...
    if options.verbose {
        println!(
            "Instrumenting code for branch coverage in {}",
            temp_path.display()
        );
    }
    ...
    let inst_data =
        instrument_for_branch_coverage(project_path, temp_path, &cfgs, &fset, &objs, options)?;
    modify_source_files(temp_path, &inst_data)?;

    // Run tests and collect coverage data
    let (covered_branches, test_output) =
        run_tests_and_collect_coverage(temp_path, options, &inst_data)?;
    ...
    if options.fail_on_low_coverage
        && result.overall_coverage_percentage < options.min_coverage_threshold
    {
        anyhow::bail!(
