...
impl BranchInstrumentation {
    ...
    pub fn instrument_file(&mut self, file_path: &Path) -> Result<String> {
        ...
        let instrumented = format!("// Instrumented version of {}\n{}", file_path.display(), content);
    ...
    pub fn export_coverage_report(&self, output_path: &Path) -> Result<()> {
        let report = CoverageReport {
            ...
            covered_branches: self.branch_data.values()
        ...
        fs::write(output_path, json)
            .with_context(|| format!("Failed to write coverage report to {}", output_path.display()))?;
...
pub fn instrument_for_branch_coverage(
    ...
    for (func_name, cfg) in cfgs {
        let function_instrumentation = instrument_function(
            func_name,
            cfg,
            fset,
            objs,
        ...
        instrumentation_data.instrumented_functions.insert(
            func_name.clone(),
...
fn instrument_function(
    ...
    for (block_id, block) in &cfg.blocks {
        ...
        if options.verbose && line == 0 {
            println!("Warning: Block {} has no precise line information", block_id);
        }
        ...
        for (succ_idx, _succ_id) in block.succs.iter().enumerate() {
            ...
            if seen_branches.insert(branch_id.clone()) {
                ...
                let specific_condition = create_specific_condition(&branch_type, &condition, succ_idx);
    ...
    if options.verbose {
        println!("Found {} branches in function {}", branches.len(), func_name);
    }
...
fn get_precise_branch_line(block: &cfg::cfg::BasicBlock, fset: &go_parser::FileSet, objs: &go_parser::AstObjects) -> usize {
...
fn get_precise_column(block: &cfg::cfg::BasicBlock, fset: &go_parser::FileSet, objs: &go_parser::AstObjects) -> usize {
...
fn analyze_block_branch_info(block: &cfg::cfg::BasicBlock, _objs: &go_parser::AstObjects) -> (String, String) {
...
fn analyze_branch_statement_detailed(stmt: &go_parser::ast::Stmt) -> Option<(String, String)> {
    ...
    match stmt {
        Stmt::If(_) => {
            Some(("if".to_string(), "if_condition".to_string()))
        },
        Stmt::Switch(_) => {
            Some(("switch".to_string(), "switch_expression".to_string()))
        },
        Stmt::TypeSwitch(_) => {
            Some(("type_switch".to_string(), "type_assertion".to_string()))
        },
        Stmt::For(_) => {
            Some(("for".to_string(), "for_condition".to_string()))
        },
        Stmt::Range(_) => {
            Some(("range".to_string(), "range_expression".to_string()))
        },
        Stmt::Select(_) => {
            Some(("select".to_string(), "channel_operations".to_string()))
        },
        _ => None,
    }
}
...
pub fn modify_source_files(
    ...
    for (file_path, instrumentations) in files_to_instrument {
        ...
        if source_file.exists() {
            ...
            modified_content = add_instrumentation_to_file(&modified_content, &combined_instrumentation)?;
            
            // Write the modified content back to the file
            fs::write(&source_file, modified_content)
                .with_context(|| format!("Failed to write instrumented file: {}", source_file.display()))?;
        }
    }
...
fn add_simplified_branch_tracking(content: &str, instrumentation: &FunctionInstrumentation) -> String {
    ...
    for branch in &instrumentation.branches {
        ...
        if parts.len() >= 3 {
            ...
            modified = add_branch_specific_tracking(&modified, func_name, &branch.branch_type, 
                                                   branch.line, succ_idx, &branch.branch_id);
        }
    }
...
fn add_branch_specific_tracking(content: &str, func_name: &str, branch_type: &str, 
                               target_line: usize, succ_idx: usize, branch_id: &str) -> String {
            ...
            match branch_type {
                "if" => {
                    ...
                    if line_content.trim().starts_with("if ") && 
                       (line_count == target_line || (line_count >= target_line - 2 && line_count <= target_line + 2)) {
                ...
                "range" | "for" => {
                    ...
                    if (line_content.contains("for ") || line_content.contains("range ")) &&
                       (line_count == target_line || (line_count >= target_line - 1 && line_count <= target_line + 1)) {
                        if succ_idx == 0 { // Loop body
                            true
                        } else {
                            false
                        }
                ...
                _ => {
                    ...
                    if (line_content.contains("if ") || line_content.contains("for ") || 
                        line_content.contains("switch ") || line_content.contains("case ")) &&
