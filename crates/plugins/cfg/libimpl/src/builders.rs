...
use crate::types;
use crate::types::{BasicBlock, ControlFlowGraph, LoopContext, Statement};
...
impl ControlFlowGraph {
    pub fn build(fset: &FileSet, func: &FuncDecl, objs: &AstObjects) -> Self {
        ...
        optimization::clean_unreachable_blocks(&mut blocks, entry);
        optimization::optimize_empty_blocks(&mut blocks, entry, exit_id, &label_def_ids);
        optimization::clean_unreachable_blocks(&mut blocks, entry);
        validation::validate_graph(&blocks, exit_id);
